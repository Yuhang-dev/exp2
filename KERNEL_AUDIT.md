...
| 路由 | mean-k 评分、相对最大分数阈值、sink/窗口/末尾保护、因果交集及排序与上游一致 |
...
1. N=1024 和 N=1089：mean-k 和归一化块评分，对齐长度与不完整末块。
...
6. N=131072：稀疏 kernel 全长执行，抽查首部、32K/64K/128K 边界位置，检查大 stride/index 下的数值。
...
`run_study.sh` 先运行检查；任一断言失败就退出，不进入模型实验。模型载入后，另核验 token 分块的 MLP/RMSNorm 与未分块计算的数值。
